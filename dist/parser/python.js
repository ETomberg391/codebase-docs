...
function parseImport(line, lineNum, imports) {
...
function parseExport(line, lineIdx, lines, exports) {
    // __all__ = ['a', 'b', 'c']
...
function parseSymbol(line, lineIdx, lines, seenSymbols, symbols) {
    const trimmed = line.trim();
...
function findBlockEndByIndent(lines, startIdx) {
...
function findAssignmentEnd(lines, startIdx) {
...
function getSnippet(lines, startIdx, endLine) {
        ...
        if (line.startsWith('"""') || line.startsWith("'''")) {
            ...
            for (let j = i + 1; j < lines.length; j++) {
                ...
                if (docLine.includes(quote)) {
                    ...
                    if (beforeQuote.trim())
                        docLines.push(beforeQuote.trim());
                    break;
                }
                if (docLine.trim())
