 ...
 */
function loadCustomIgnore(basePath) {
    const patterns = [];
    const negations = [];
 ...
 */
function shouldIgnoreByPatterns(relPath, isDirectory, customPatterns, customNegations) {
 ...
 */
export function scanDirectory(basePath, options = {}) {
    ...
    const allPatterns = [
        ...
        ...customPatterns,
        ...(options.ignoreDirs ?? []).map((d) => d.endsWith('/') ? d : d + '/'),
    ];
