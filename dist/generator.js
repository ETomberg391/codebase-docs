...
export function generate(result, options = {}) {
    ...
    // Generate the HTML
    const html = buildHtml(title, { ...result, modules: modulesWithoutContent }, graphData, theme);
    // Write to file
    const resolvedPath = path.resolve(outputPath);
    fs.writeFileSync(resolvedPath, html, 'utf-8');
    return resolvedPath;
}
function buildHtml(title, result, graphData, theme) {
