...
export function buildGraph(
  ...
  // Build nodes
  const nodes: GraphNode[] = modules.map(mod => {
...
function computeLayout(nodes: GraphNode[], edges: GraphEdge[]): void {
  ...
  for (let iter = 0; iter < maxIterations; iter++) {
    ...
    if (useSpatialGrid) {
      ...
      for (const node of nodes) {
        ...
        for (const other of neighbors) {
          ...
          if (distSq > gridSize * gridSize * 4) continue;
    ...
    for (const edge of edges) {
      ...
      if (fromForce) { fromForce.fx += fx; fromForce.fy += fy; }

      const toForce = forces.get(toNode.id);
      if (toForce) { toForce.fx -= fx; toForce.fy -= fy; }
    }
...
// Spatial grid helpers for optimized repulsion
interface GridCell {
  row: number;
  col: number;
}
...
function buildSpatialGrid(nodes: GraphNode[], gridSize: number): SpatialGrid {
  const cols = Math.ceil(800 / gridSize);
  const rows = Math.ceil(600 / gridSize);
  const cells: GraphNode[][][] = Array.from({ length: rows }, () =>
...
function getGridCell(x: number, y: number, gridSize: number, width: number, height: number): GridCell {
...
function getGridNeighbors(grid: SpatialGrid, row: number, col: number): GraphNode[] {
